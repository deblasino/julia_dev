...
# Utilizziamo la coordinata "AttractionPoint" che rappresenta la posizione del cursore.
...
# Componenti per l'interazione con l'ostacolo (invariati)
...
add_component!(vms, GaussianSpring("UpperError"; stiffness=-150.0, width=0.1); id="UpperObstacleSpring")
add_component!(vms, LinearDamper(20.0, "UpperError"); id="UpperObstacleDamper")
add_component!(vms, GaussianSpring("LowerError"; stiffness=-150.0, width=0.1); id="LowerObstacleSpring")
add_component!(vms, LinearDamper(20.0, "LowerError"); id="LowerObstacleDamper")
...
# Definiamo il vettore-asse della sbarra
...
# === CORREZIONE 1: Aggiunta la coordinata 'BarAxisVector' a 'vm' ===
add_coordinate!(vm, CoordDifference("BarAxisPoint", "BarOrigin"); id="BarAxisVector")

# Definiamo il vettore-asse Z del TCP
...
# === CORREZIONE 2: Aggiunta la coordinata 'ToolAxisVector' a 'robot' ===
add_coordinate!(robot, CoordDifference("ToolAxisPoint", "ToolOrigin"); id="ToolAxisVector")

# Molla che minimizza la differenza tra i due vettori
...
# Il cursore parte a 0.25m dal centro (l'estremità superiore della sbarra) 
...
mesh!(ls, Sphere(Point3f(pivot_point), 0.03), color=(:green, 0.9), label="Target")
mesh!(ls, Sphere(Point3f(obstacle_position), 0.04), color=(:orange, 0.9), label="Ostacolo")
...
# Visualizziamo il cursore mobile come una sfera blu sulla sbarra
scatter!(ls, attraction_pos, color=:blue, markersize=20, label="Punto di Contatto")

# Creazione dell'animazione
savepath = joinpath(@__DIR__, "franka_sliding_contact.mp4")
