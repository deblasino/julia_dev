...
# === COMPONENTI PER L'INTERAZIONE CON L'OSTACOLO DINAMICO ===
obstacle_position_1 = SVector(0.4, 0.2, 0.5)

# MODIFICA: Uso di ReferenceCoord per permettere l'aggiornamento della posizione (sciurus example)
println("3.1. Creazione dell'ostacolo dinamico...")
...
# === COMPONENTI PER L'EVITAMENTO DEGLI OSTACOLI MULTI-LINK (Semplificato) ===
links_da_proteggere = ["fr3_link4","fr3_link5", "fr3_link6", "fr3_link7", "fr3_link8"]
# MODIFICA: Rimosso l'ostacolo 2
ostacoli = [
...
# MODIFICA: La logica ora si basa solo sulla posizione iniziale dell'ostacolo 1
obstacle_positions = [obstacle_position_1] 
...
# =======================================================================================
# --- 4. SIMULAZIONE CON OSTACOLO DINAMICO ---
# =======================================================================================
println("4. Avvio della simulazione con ostacolo dinamico...")
tspan = (0.0, 15.0)
...
# MODIFICA: Definizione della traiettoria dell'ostacolo e delle callback per la simulazione
base_obstacle_pos = obstacle_position_1
amplitude = 0.15 # Metri
frequency = 0.2 # Hertz (oscillazioni al secondo)

"""
Definisce la posizione dell'ostacolo 1 in funzione del tempo.
"""
obstacle_1_pos(t) = base_obstacle_pos + SVector(0.0, amplitude * sin(2 * π * frequency * t),0.0)

"""
Funzione di setup: chiamata una volta prima della simulazione per ottenere i riferimenti.
"""
function f_setup(cache)
    # Ottiene l'ID della coordinata dell'ostacolo
    obstacle_coord_id = get_compiled_coordID(cache, ".virtual_mechanism.ObstaclePosition1")
    # Ottiene il riferimento (RefValue) al valore della coordinata
    obstacle_pos_ref = cache[obstacle_coord_id].coord_data.val
    return (obstacle_pos_ref,) # Restituisce una tupla con i riferimenti
end
...
function f_control(cache, t, args, extra)
    (obstacle_pos_ref,) = args # Estrae i riferimenti dalla tupla
    obstacle_pos_ref[] = obstacle_1_pos(t) # Aggiorna la posizione dell'ostacolo
    return false
end

compiled_vms = compile(vms)
dcache = new_dynamics_cache(compiled_vms)
# MODIFICA: Passaggio delle funzioni f_setup e f_control a get_ode_problem
...
# --- 5. VISUALIZZAZIONE (Corretto v4 - Funzione Giusta) ---
...
# CORREZIONE 4: Usiamo meshscatter!
meshscatter!(
    ls,
    obs1_pos,
...
savepath = joinpath(@__DIR__, "franka_dynamic_avoidance.mp4")
