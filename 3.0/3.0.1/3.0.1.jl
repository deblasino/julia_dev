...
# --- FIX: Aggiunta coordinate per TUTTI e 3 gli ostacoli ---
...
add_coordinate!(vm, ConstCoord(OBSTACLE_POSITION_3); id="ObstaclePosition3") # Aggiunto

# --- FIX: Aggiornamento lista ostacoli per la repulsione segmenti ---
...
for i in 1:NUM_ACTUAL_SEGMENTS
    ...
    for (j, ostacolo_id) in enumerate(ostacoli_vm)
        # Genera ID univoci per molle/errori basati sull'indice dell'ostacolo
...
# Repulsione Robot (Invariata ma estesa al 3° ostacolo)
links_robot = ["fr3_link3","fr3_link4", "fr3_link5", "fr3_link6", "fr3_link7", "fr3_link8", "fr3_hand"]

# --- FIX: Aggiunto l'ostacolo 3 alla configurazione del robot fisico ---
ostacoli_robot_cfg = [
    (id=1, c=".virtual_mechanism.ObstaclePosition1"), 
    (id=2, c=".virtual_mechanism.ObstaclePosition2"),
    (id=3, c=".virtual_mechanism.ObstaclePosition3") # Aggiunto
]
...
#    - LinearSpring: Stiffness 200.0 N/m (o superiore) per mantenere la punta vicina all'ancoraggio.
