...
actuated_arm_joints = ["fr3_joint$(i)" for i in 1:7]
for joint_name in actuated_arm_joints
    add_coordinate!(robot, JointSubspace(joint_name); id="$(joint_name)_coord")
    add_component!(robot, LinearDamper(0.1, "$(joint_name)_coord"); id="$(joint_name)_damper")
end
...
for i in 1:NUM_ACTUAL_SEGMENTS
    ...
    for (j, ostacolo_id) in enumerate(ostacoli_vm)
        ...
        add_component!(vms, GaussianSpring("Seg$(i)_Err$(j)"; stiffness=-50.0, width=0.08); id="Seg$(i)_Spr$(j)") 
...
for lnk in links_robot
    ...
    for obs in ostacoli_robot_cfg
        ...
        add_component!(vms, GaussianSpring("$(lnk)_ObsErr$(obs.id)"; stiffness=-150.0, width=0.11); id="$(lnk)_ObsSpr$(obs.id)")
    end
end
...
add_component!(vms, TanhSpring("SliderDrivingError_s"; max_force=15.0, stiffness=500.0); id="SliderDrivingSpring") 
...
add_component!(vms, TanhSpring("SliderFeedbackError_s"; max_force = 15.0, stiffness=1000.0); id="SliderFeedbackSpring")
...
add_component!(vms, TanhSpring("PathError"; max_force=15.0, stiffness=1000.0); id="PathSpring")
D_mat = SMatrix{3,3}(25.0,0,0, 0,25.0,0, 0,0,25.0)
add_component!(vms, LinearDamper(D_mat, "PathError"); id="PathDamper")
...
D_roll = SMatrix{3,3}(20.,0,0, 0,20.,0, 0,0,20.)
for i in 1:3
    add_component!(vms, TanhSpring("SErr$i"; max_force=10.0, stiffness=150.0); id="SSpr$i")
    add_component!(vms, LinearDamper(D_roll, "SErr$i"); id="SDmp$i") 
end
