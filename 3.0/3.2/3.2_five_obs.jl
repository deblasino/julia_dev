...
const OBSTACLE_POSITION_1 = SVector(0.4, 0.1, 0.2)
const OBSTACLE_POSITION_2 = SVector(0.6, -0.1, 0.2)
const OBSTACLE_POSITION_3 = SVector(0.7, 0.0, 0.2)
const OBSTACLE_POSITION_4 = SVector(1.55, -0.15, 0.2)
const OBSTACLE_POSITION_5 = SVector(1.55, 0.15, 0.3)
const OBSTACLES_LIST = [OBSTACLE_POSITION_1, OBSTACLE_POSITION_2, OBSTACLE_POSITION_3, OBSTACLE_POSITION_4, OBSTACLE_POSITION_5]
...
const TARGET_TOLERANCE = 0.15    # 10 cm di tolleranza per smettere di aggiungere segmenti
...
# Definizione punti sul gripper per il controllo di orientamento
...
# Definizione TCP centrale per il controllo di posizione
...
# =======================================================================================
# --- 2. VIRTUAL MECHANISM DEFINITION (Generazione Dinamica con Lunghezza Variabile) ---
# =======================================================================================
println("2. Creazione della 'Snake-Bar' dinamica estensibile ($NUM_ACTUAL_SEGMENTS segmenti)...")

# Inerzia nominale (approssimata per la lunghezza media)
...
for i in 1:NUM_ACTUAL_SEGMENTS
    # Creazione Frames
    joint_frame = add_frame!(vm, "joint$(i)_frame") # Frame BASE del prismatico
    seg_frame = add_frame!(vm, "seg$(i)_frame")     # Frame PUNTA del prismatico (mobile)
...
# =======================================================================================
# --- 3. VMS ASSEMBLY (Completo: Ostacoli + Estensione) ---
# =======================================================================================
println("3. Setup VMS...")
vms = VirtualMechanismSystem("SlidingControl", robot, vm)

# 1. Definizione Ostacoli nel VMS
...
add_coordinate!(vm, ConstCoord(OBSTACLE_POSITION_4); id="ObstaclePosition4")
add_coordinate!(vm, ConstCoord(OBSTACLE_POSITION_5); id="ObstaclePosition5")
ostacoli_vm = ["ObstaclePosition1", "ObstaclePosition2", "ObstaclePosition3", "ObstaclePosition4", "ObstaclePosition5"]

# 2. REPULSIONE SEGMENTI (Ciclo per evitare gli ostacoli)
# Questa è la parte che probabilmente mancava: applica forze laterali ai segmenti
for i in 1:NUM_ACTUAL_SEGMENTS
    seg_pos_id = "Seg$(i)_Pos"
    for (j, ostacolo_id) in enumerate(ostacoli_vm)
        # Calcola vettore differenza tra segmento e ostacolo
        add_coordinate!(vms, CoordDifference(".virtual_mechanism.$(seg_pos_id)", ".virtual_mechanism.$(ostacolo_id)"); id="Seg$(i)_Err$(j)")
        
        # Molla Gaussiana Repulsiva (Stiffness negativa = spinge via)
        # Nota: Puoi aumentare l'ampiezza (width) o la forza (stiffness) se vuoi più reattività
        add_component!(vms, GaussianSpring("Seg$(i)_Err$(j)"; stiffness=-80.0, width=0.08); id="Seg$(i)_Spr$(j)") 
        
        # Smorzamento per evitare oscillazioni eccessive quando si avvicina
        # add_component!(vms, LinearDamper(5.0, "Seg$(i)_Err$(j)"); id="Seg$(i)_Dmp$(j)")
    end
end

# 3. CONTROLLO LONGITUDINALE (Molle Telescopiche)
# Questa è la parte nuova per gestire l'allungamento/accorciamento
for i in 1:NUM_ACTUAL_SEGMENTS
    # Definizione coordinata giunto (lunghezza attuale)
    if !haskey(vm.coordinates, "Seg$(i)_Len_Q") # Controllo sicurezza se già aggiunto in 2
         add_coordinate!(vm, JointSubspace("J_seg$(i)_length"); id="Seg$(i)_Len_Q")
    end
    
    # Definizione lunghezza nominale (Target = SEGMENT_LENGTH)
    # Vogliamo che lo snake "preferisca" stare alla lunghezza originale, ma possa cambiare se forzato
    add_coordinate!(vm, ConstCoord(SVector(SEGMENT_LENGTH)); id="Seg$(i)_Nominal_Len")
    
    # Errore (Differenza tra lunghezza attuale e nominale)
    add_coordinate!(vms, CoordDifference(".virtual_mechanism.Seg$(i)_Len_Q", ".virtual_mechanism.Seg$(i)_Nominal_Len"); id="Seg$(i)_Len_Err")
    
    # Molla e Smorzatore Longitudinale
    # Stiffness: 200.0 è un buon compromesso. Se troppo bassa, lo snake si "smolla". Se troppo alta, diventa rigido.
...
# 4. REPULSIONE ROBOT (Evitamento ostacoli per il braccio reale)
links_robot = ["fr3_link3","fr3_link4", "fr3_link5", "fr3_link6", "fr3_link7", "fr3_link8", "fr3_hand"]
ostacoli_robot_cfg = [(id=1, c=".virtual_mechanism.ObstaclePosition1"), (id=2, c=".virtual_mechanism.ObstaclePosition2"), (id=3, c=".virtual_mechanism.ObstaclePosition3"), (id=4, c=".virtual_mechanism.ObstaclePosition4"), (id=5, c=".virtual_mechanism.ObstaclePosition5")]
...
# 2. Definiamo la coordinata della posizione attuale della punta
add_coordinate!(vm, FrameOrigin(last_seg_frame_name); id="Snake_Tip_Pos")

# 3. Definiamo il punto di ancoraggio fisso
#    Usiamo P_EE_INITIAL, che è il punto target usato dal generatore in Sezione 1.5
add_coordinate!(vm, ConstCoord(P_EE_INITIAL); id="Snake_Tip_Anchor_Ref")

# 4. Calcoliamo l'errore (distanza tra punta attuale e ancoraggio)
...
# 5. Applichiamo una Molla "Rigida" e uno Smorzatore
#    - Stiffness (k): Valore alto (es. 2000.0 o 5000.0) per simulare un vincolo rigido.
#    - Damping (d): Necessario per dissipare l'energia vibrazionale che una molla rigida introduce.
...
# 5. CONTROLLO SLIDER (Drive del cursore)
...
# 6. FEEDBACK ROBOT SU SLIDER (Accoppiamento)
# Definiamo una coordinata di riferimento che conterrà il valore 's' corrente del robot
...
# 7. INSEGUIMENTO PERCORSO (Robot segue Snake)
...
# =======================================================================================
# --- 4.5 CALCOLO STATO INIZIALE VM (CORRETTO PER SNAKE ESTENSIBILE) --- 
# =======================================================================================
...
# Il primo segmento è connesso tramite J_base_yaw(Z) -> J_base_pitch(Y) -> J_roll_base(X) -> J_seg1_length(Prism)
...
# --- MODIFICA FONDAMENTALE: Inserimento della lunghezza nel vettore ---
# Struttura Seg 1: [Yaw, Pitch, Roll, LUNGHEZZA]
q_vm_components = Float64[q_yaw_base, q_pitch_base, q_roll_base, SEGMENT_LENGTH]

# Matrice di rotazione accumulata (solo rotazioni, la traslazione è gestita dalla struttura)
...
for i in 2:NUM_ACTUAL_SEGMENTS
    ...
    # Calcoliamo angoli relativi
    q_rel_yaw, q_rel_pitch = vector_to_yaw_pitch(v_local) 
    
    # --- MODIFICA FONDAMENTALE: Inserimento della lunghezza ---
    # Struttura Seg i: [Yaw, Pitch, LUNGHEZZA]
    push!(q_vm_components, q_rel_yaw)
    push!(q_vm_components, q_rel_pitch)
    push!(q_vm_components, SEGMENT_LENGTH) # <-- Qui aggiungiamo il DOF mancante!
...
println("   Stato iniziale VM calcolato ($length(q_vm_initial) DOF). Previsto: 11 (per 3 segmenti).")
...
mesh!(ls, Sphere(Point3f(OBSTACLE_POSITION_3), 0.04), color=:cyan, label="Ostacolo 3")
mesh!(ls, Sphere(Point3f(OBSTACLE_POSITION_4), 0.04), color=:yellow, label="Ostacolo 4")
mesh!(ls, Sphere(Point3f(OBSTACLE_POSITION_5), 0.04), color=:red, label="Ostacolo 5")
...
for i in 1:NUM_ACTUAL_SEGMENTS
    ...
    # 2. Observable per i punti base e punta attuale
    p_base = map(c -> Point3f(configuration(c, base_id)), pkcache)
    p_tip  = map(c -> Point3f(configuration(c, tip_id)), pkcache)
    
    # 3. Calcolo del punto "Nominale" (Dove finirebbe il segmento se non fosse allungato)
    #    SEGMENT_LENGTH è la lunghezza a riposo definita all'inizio (es. 0.15)
    ...
    p_nominal_tip = map((b, t) -> begin
        ...
        dist = norm(vec)
        if dist < 1e-6
            return b + Point3f(0, -L_nom, 0) # Fallback se dist=0
        else
            dir = vec / dist
            # Il punto nominale è a distanza L_nom dalla base, lungo la direzione attuale
            return b + dir * L_nom 
        end
    end, p_base, p_tip)

    # 4. Disegno del cilindro di ALLUNGAMENTO
    #    Parte da p_nominal_tip e arriva a p_tip.
    #    La sua lunghezza è esattamente (Lunghezza_Reale - Lunghezza_Nominale).
    #    Lo facciamo di un colore diverso (es. Giallo/Arancio) per evidenziare lo sforzo.
    
    # Cilindro deformazione (visibile solo se c'è allungamento/compressione)
    mesh!(ls, map((p_nom, p_real) -> Cylinder(p_nom, p_real, 0.02f0), p_nominal_tip, p_tip), 
          color=(:orange, 0.6), transparency=true)
          
    # 5. Riferimento visivo della lunghezza nominale (Opzionale: Cilindro fantasma grigio)
    #    Utile per vedere "quanto dovrebbe essere lungo".
