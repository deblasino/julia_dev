# =======================================================================================
# SIMULAZIONE VMC IBRIDA - SBARRA "SNAKE" A LUNGHEZZA VARIABILE (TELESCOPICA)
# =======================================================================================
...
# --- Ostacoli ---
const OBSTACLE_POSITION_1 = SVector(0.3, 0.15, 0.15)
const OBSTACLE_POSITION_2 = SVector(0.4, -0.15, 0.45)
...
# Parametri Molle/Smorzatori (Devono essere identici in Fase 1 e Fase 2 per coerenza)
const K_TELESCOPIC = 50.0       # Rigidità molla lunghezza (tiene i segmenti vicini a L_NOMINAL)
const D_TELESCOPIC = 5.0        # Smorzamento allungamento
const K_ATTR_TARGET = 80.0       # Attrazione della punta verso il Target (P_EE_INITIAL)
...
# =======================================================================================
# --- 1.5 GENERAZIONE TOPOLOGIA TELESCOPICA (CORRETTO) ---
# =======================================================================================
println("1.5. Generazione Topologia tramite Simulazione Fisica (Telescopica)...")
...
for i in 1:NUM_SEGMENTS_DESIRED
    ...
    # 1. Frames: Base Giunto e Punta Segmento (Mobile)
    joint_frame = add_frame!(vm_gen, "gen_j$(i)")
    s_frame = add_frame!(vm_gen, "gen_seg$(i)") # Punta che scorre
    
    # 2. Giunti Rotazionali (Yaw + Pitch)
    ...
    # 3. GIUNTO PRISMATICO (Estensione Lungo Y negativo)
    prism_id = "Gen_Prism$(i)"
    add_joint!(vm_gen, Prismatic(SVector(0.0, -1.0, 0.0)); parent=joint_frame, child=s_frame, id=prism_id)

    # 4. Molla Telescopica (Mantiene L_NOMINAL)
    # Definizione coordinata fisica
    prism_coord_id = "Gen_Len_Phys_Coord$(i)"
    add_coordinate!(vm_gen, JointSubspace(prism_id); id=prism_coord_id)
    
    # --- FIX FONDAMENTALE: MASSA TRASLAZIONALE ---
    # Senza questo, F=kx ma m=0 -> a=inf -> NaN
    add_component!(vm_gen, LinearInerter(SEG_MASS, prism_coord_id); id="Gen_Len_Inertia$(i)") 
    add_component!(vm_gen, LinearDamper(D_TELESCOPIC * 2.0, prism_coord_id); id="Gen_Len_Phys_Damp$(i)") # Smorzamento extra per stabilità
    # ----------------------------------------------

    # Molla di ritorno a lunghezza nominale
    ...
    # 5. Componenti Inerziali ROTAZIONALI
    ...
    # 6. Posizione Punta (per repulsione/attrazione)
    add_coordinate!(vm_gen, FrameOrigin(s_frame); id="Gen_Pos$(i)")

    # 7. Repulsione Ostacoli (Identica alla Fase 2 per consistenza)
    for (k, obs) in enumerate(OBSTACLES_LIST)
        obs_name = "Gen_Obs$(i)_$(k)" # Nome univoco
    ...
    # 8. Attrazione al Target (Solo sulla PUNTA ATTUALE per guidare la crescita)
    if i > 1
        # Rimuoviamo l'attrazione dal segmento precedente per evitare conflitti eccessivi
        if haskey(vm_gen.components, "Gen_Attract$(i-1)")
            delete!(vm_gen.components, "Gen_Attract$(i-1)")
        end
        # "Congeliamo" parzialmente il segmento precedente aumentando lo smorzamento
        # (Opzionale, ma aiuta la convergenza)
    end
    ...
    local q_start
    if i == 1
        # [YawBase, PitchBase, Lunghezza]
        q_start = [0.0, 0.0, L_NOMINAL] 
    else
        # [q_prev..., YawRel, PitchRel, Lunghezza]
        q_start = [q_prev; 0.0; 0.0; L_NOMINAL]
    end
    
    cache = new_dynamics_cache(m_step)
    
    # Tempo più lungo per permettere l'assestamento delle lunghezze
...
# Estrazione Dati: Yaw, Pitch e Lunghezza per ogni segmento
global idx = 1  # FIX: Scope globale esplicito
# Primo Segmento: 3 DoF [Yaw, Pitch, Lunghezza]
push!(gen_results_angles, (q_prev[idx], q_prev[idx+1]))
push!(gen_results_lengths, q_prev[idx+2])
global idx += 3

# Segmenti successivi: 3 DoF ciascuno [Yaw, Pitch, Lunghezza]
for i in 2:NUM_SEGMENTS_DESIRED
    global idx # Riferimento alla variabile globale
    push!(gen_results_angles, (q_prev[idx], q_prev[idx+1]))
    push!(gen_results_lengths, q_prev[idx+2])
    idx += 3
end
...
for i in 1:NUM_SEGMENTS_DESIRED
    ...
    if i == 1
        # Mappatura Pitch Generatore -> Roll VMC (come da configurazione originale)
    ...
    # GIUNTO PRISMATICO
    add_joint!(vm, Prismatic(SVector(0.0, -1.0, 0.0)); parent=j_frame, child=s_frame, id=prism_id)
    ...
    # Molla Telescopica (Stessi parametri Generatore!)
    ...
    #add_coordinate!(vm, CoordDifference(".virtual_mechanism.Seg$(i)_Len_Q", ".virtual_mechanism.Seg$(i)_Nominal_Len"); id="Seg$(i)_Len_Err")
    
    # Coord Visualizzazione
    add_coordinate!(vm, FrameOrigin(s_frame); id="Seg$(i)_Pos")
    add_coordinate!(vm, FrameOrigin(j_frame); id="Seg$(i)_Base_Pos") # Per visualizzare il cilindro
...
# FIX: Inizializziamo la posizione visiva del cursore esattamente sul target (P_EE_INITIAL)
# Invece di calcolarla come "sbarra dritta in giù" (che risultava dietro il robot).
initial_slider_pos_world = P_EE_INITIAL 

add_coordinate!(vm, ReferenceCoord(Ref(initial_slider_pos_world)); id="SliderWorldPos_Ref")

# Anche i target di orientamento (il triangolo) devono partire dalla posizione corretta
...
# --- REPLICA FORZE GENERATORE NEL VMC ---
...
# 2. Repulsione Ostacoli (Stessi parametri Generatore)
for (k, obs) in enumerate(OBSTACLES_LIST)
...
# 3. ATTRAZIONE AL TARGET (Stabilizzazione Percorso)
# Manteniamo la molla che tira la punta finale verso P_EE_INITIAL, bilanciando le altre forze.
...
# --- INTERAZIONE ROBOT ---
# Repulsione Robot dagli ostacoli
...
for lnk in links_robot
    ...
    for (k, obs) in enumerate(OBSTACLES_LIST)
        ...
        add_component!(vms, GaussianSpring("$(lnk)_ObsErr$(k)"; stiffness=-150.0, width=0.11); id="$(lnk)_ObsSpr$(k)")
    end
end

# Controllo Slider (Drive)
...
# Inseguimento Percorso (Robot segue Slider)
...
function f_control(cache, t, args, extra)
    ...
    # 1. Recupero geometria percorso attuale
    ...
    # 2. Proiezione Robot su Catena (Invariato)
    p_robot = configuration(cache, tcp_id)
    best_dist_sq = Inf
    s_robot_projected = 0.0
    
    # Per la proiezione usiamo ancora una stima o iteriamo sulle lunghezze reali
    current_s_proj = 0.0
    for k in 1:(length(points_path)-1)
        # Recuperiamo la lunghezza reale del segmento k
        # Nota: gen_results_lengths deve essere accessibile qui (è globale nello script)
    ...
    current_s = 0.0
    seg_index = 1
    ratio = 0.0  # <--- INIZIALIZZAZIONE FONDAMENTALE
    
    # Iteriamo sulle lunghezze reali per trovare in quale segmento siamo
    for i in 1:NUM_SEGMENTS_DESIRED
        len_i = gen_results_lengths[i]
        
        # Se s cade in questo segmento OPPURE è l'ultimo segmento (gestione fine corsa)
    ...
    # Ora ratio è definito e utilizzabile qui fuori
...
# =======================================================================================
# --- 4.5 ASSEMBLAGGIO STATO INIZIALE (CON LUNGHEZZE ESTRATTE) --- 
# =======================================================================================
println("4.5. Assemblaggio stato iniziale...")

q_vm_components = Float64[]

# Base (Seg 1): [Yaw, Pitch, Roll, Lunghezza1]
push!(q_vm_components, gen_results_angles[1][1]) # Yaw
push!(q_vm_components, 0.0)                      # Pitch
push!(q_vm_components, gen_results_angles[1][2]) # Roll
push!(q_vm_components, gen_results_lengths[1])   # Lunghezza!

# Segmenti successivi: [Yaw, Pitch, Lunghezza_i]
for i in 2:NUM_SEGMENTS_DESIRED
    push!(q_vm_components, gen_results_angles[i][1]) # Yaw Rel
    push!(q_vm_components, gen_results_angles[i][2]) # Pitch Rel
    push!(q_vm_components, gen_results_lengths[i])   # Lunghezza!
end

push!(q_vm_components, BAR_LENGTH_TOTAL) # Slider pos
