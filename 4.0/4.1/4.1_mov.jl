...
const PIVOT_POINT = SVector(0.6, 0.0, 0.1) # Aggiornato Z a 0.1 come in 4.0

# --- OSTACOLI (Da 4.0_mov.jl) ---
...
# --- PARAMETRI MOVIMENTO OSTACOLI (Da 4.0_mov.jl) ---
...
# =======================================================================================
# --- 1.5 GENERAZIONE TOPOLOGIA TELESCOPICA ---
# =======================================================================================
...
for i in 1:NUM_SEGMENTS_DESIRED
    ...
    # Repulsione Ostacoli (Usiamo ConstCoord per la generazione statica)
    ...
    prob = get_ode_problem(cache, VMRobotControl.DEFAULT_GRAVITY, q_start, zero_q̇(m_step), (0.0, 8.0))
    sol = solve(prob, Tsit5(); abstol=1e-5, reltol=1e-5)
    q_prev = sol.u[end][1:config_size(m_step)]
end

println("   Generazione completata. Estrazione configurazione...")
...
# --- DEFINIZIONE OSTACOLI DINAMICI NEL VMS (Da 4.0_mov.jl) ---
...
# 2. Repulsione Ostacoli (Segmenti)
# Loop semplificato che usa le coordinate già definite
for k in 1:3
...
# =======================================================================================
# --- 4. LOGICA DI CONTROLLO (MODIFICATA PER OSTACOLI MOBILI) ---
# =======================================================================================
...
function f_control(cache, t, args, extra)
    ...
    # --- MOVIMENTO OSTACOLI (Da 4.0_mov.jl) ---
...
# --- VISUALIZZAZIONE OSTACOLI MOBILI (Da 4.0_mov.jl) ---
...
mesh!(ls, map(p -> Sphere(p, 0.04), obs1_pos_obs), color=:orange, label="Ostacolo 1 (Mobile)")
mesh!(ls, map(p -> Sphere(p, 0.04), obs2_pos_obs), color=:magenta, label="Ostacolo 2 (Mobile)")
