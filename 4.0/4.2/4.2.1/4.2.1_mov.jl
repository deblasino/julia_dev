# =======================================================================================
# SIMULAZIONE VMC IBRIDA - SNAKE RICORSIVO ADATTIVO CON OSTACOLI DINAMICI
# (File 4.2 Modificato con logica ostacoli di 4.1_mov_v)
# =======================================================================================
...
# --- MODIFICA: OSTACOLI DINAMICI (Presi da 4.1_mov_v.jl) ---
...
const OBS_AMP  = 0.30 # Ampiezza movimento (m)

# --- PARAMETRI SNAKE DINAMICI ---
const MAX_SEGMENTS = 7         
const L_SEGMENT_DEFAULT = 0.15 
...
# =======================================================================================
# --- 1.5 GENERAZIONE TOPOLOGIA RICORSIVA (GENERATORE STATICO) ---
# =======================================================================================
# Nota: Durante la generazione manteniamo gli ostacoli statici (nominali) per calcolare la topologia.
...
while !reached_target && num_generated_segments < MAX_SEGMENTS
    ...
    # Repulsione Ostacoli (STATICHE nel generatore)
    ...
    add_component!(vm_gen, TanhSpring("Gen_AttrErr$(i)"; stiffness=K_ATTR_TARGET, max_force=10.0); id="Gen_Attract$(i)")
    ...
    tolleranza_estensione = L_SEGMENT_DEFAULT * 1.30 
...
# =======================================================================================
# --- 2. VMS ASSEMBLY (Finale con Ostacoli DINAMICI) ---
# =======================================================================================
...
# 2. Repulsione Ostacoli (MODIFICATO: Usiamo ReferenceCoord per permettere il movimento)
# In VMRobotControl, per modificare le coordinate in f_control, devono essere ReferenceCoord
for (k, obs) in enumerate(OBSTACLES_LIST)
    obs_name = "ObstaclePosition$(k)"
    # Usiamo ReferenceCoord invece di ConstCoord per permettere aggiornamenti dinamici
    add_coordinate!(vm, ReferenceCoord(Ref(obs)); id=obs_name)
...
for lnk in links_robot
    ...
    for (k, obs) in enumerate(OBSTACLES_LIST)
        ...
        add_component!(vms, GaussianSpring("$(lnk)_ObsErr$(k)"; stiffness=-350.0, width=0.1); id="$(lnk)_ObsSpr$(k)")
    end
end

# Controllo Slider e Path
...
# =======================================================================================
# --- 4. LOGICA DI CONTROLLO (MODIFICATA PER OSTACOLI DINAMICI) ---
# =======================================================================================
function f_setup(cache)
    ...
    # --- MODIFICA: Recupero Riferimenti Ostacoli ---
    # Poiché abbiamo usato ReferenceCoord, coord_data.val è un RefValue che possiamo modificare in place
    obs_refs = [cache[get_compiled_coordID(cache, ".virtual_mechanism.ObstaclePosition$k")].coord_data.val for k in 1:length(OBSTACLES_LIST)]
    ...
    return (swp_ref, p1_ref, p2_ref, p3_ref, obs_refs, slider_s_id, pivot_id, seg_ids_compiled, rob_prog_ref, tcp_id)
end

function f_control(cache, t, args, extra)
    (swp_ref, p1_ref, p2_ref, p3_ref, obs_refs, slider_s_id, pivot_id, seg_ids_compiled, rob_prog_ref, tcp_id) = args

    # --- MODIFICA: AGGIORNAMENTO POSIZIONE OSTACOLI (Logica da 4.1_mov_v) ---
    delta_val = OBS_AMP * sin(OBS_FREQ * t)
    
    # Ostacolo 1: Movimento asse Y
    if length(obs_refs) >= 1
        obs_refs[1][] = OBSTACLE_POSITION_1 + SVector(0.0, delta_val, 0.0)
    end
    
    # Ostacolo 2: Movimento asse X
    if length(obs_refs) >= 2
        obs_refs[2][] = OBSTACLE_POSITION_2 - SVector(delta_val, 0.0, 0.0)
    end
    # Ostacolo 3 rimane statico (è nella lista ma non lo muoviamo)
    # ---------------------------------------------------------------------
...
# --- MODIFICA: VISUALIZZAZIONE OSTACOLI DINAMICI ---
# Invece di usare le costanti statiche, leggiamo la posizione aggiornata dalla cache
colors_obs = [:orange, :magenta, :cyan]
for k in 1:length(OBSTACLES_LIST)
    obs_id = get_compiled_coordID(compile(vms), ".virtual_mechanism.ObstaclePosition$k")
    obs_pos_obs = map(c -> Point3f(configuration(c, obs_id)), pkcache)
    mesh!(ls, map(p -> Sphere(p, 0.04), obs_pos_obs), color=colors_obs[k], label="Ostacolo $k")
end
...
savepath = joinpath(@__DIR__, "franka_recursive_snake_dynamic.mp4")
