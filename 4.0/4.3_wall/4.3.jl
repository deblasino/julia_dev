...
"""
Genera una lista di punti 3D che formano un muro lineare tra `p_start` e `p_end`.
`step` deve essere minore della larghezza (width) della GaussianSpring per evitare "buchi".
"""
function create_wall_points(p_start::SVector{3, Float64}, p_end::SVector{3, Float64}, step::Float64)
    dir_vec = p_end - p_start
    total_len = norm(dir_vec)
    num_points = ceil(Int, total_len / step)
    ...
    for i in 0:num_points
        ...
        p = p_start + dir_norm * (i * step)
        # Tronca se superiamo la fine (a causa dell'arrotondamento di ceil)
        if norm(p - p_start) > total_len 
            push!(points, p_end)
            break 
        end
        push!(points, p)
...
# Muro 1: Verticale lungo l'asse Z, spostato in X=0.4, Y=0.2
const WALL_1_START = SVector(0.4, 0.0, 0.0)
const WALL_1_END   = SVector(0.4, 0.0, 0.9)

# Muro 2: Orizzontale, blocca il passaggio in basso a sinistra
const WALL_2_START = SVector(0.4, -0.15, 0.0)
const WALL_2_END   = SVector(0.4, -0.15, 0.9)

const WALL_4_START = SVector(0.4, -0.30, 0.0)
const WALL_4_END   = SVector(0.4, -0.30, 0.9)

const WALL_6_START = SVector(0.4, -0.45, 0.0)
const WALL_6_END   = SVector(0.4, -0.45, 0.9)

const WALL_8_START = SVector(0.4, -0.60, 0.0)
const WALL_8_END   = SVector(0.4, -0.60, 0.9)

const WALL_10_START = SVector(0.4, 0.1, 0.0)
const WALL_10_END   = SVector(0.4, 0.1, 0.3)

const WALL_12_START = SVector(0.4, 0.25, 0.0)
const WALL_12_END   = SVector(0.4, 0.25, 0.3)

const WALL_14_START = SVector(0.4, 0.40, 0.0)
const WALL_14_END   = SVector(0.4, 0.40, 0.3)

const WALL_16_START = SVector(0.4, 0.1, 0.7)
const WALL_16_END   = SVector(0.4, 0.1, 0.9)

const WALL_18_START = SVector(0.4, 0.25, 0.7)
const WALL_18_END   = SVector(0.4, 0.25, 0.9)

const WALL_20_START = SVector(0.4, 0.40, 0.0)
const WALL_20_END   = SVector(0.4, 0.40, 0.9)

const WALL_22_START = SVector(0.4, 0.55, 0.0)
const WALL_22_END   = SVector(0.4, 0.55, 0.9)
...
# Generazione della lista completa di "sferette" che compongono i muri
const WALL_POINTS_1 = create_wall_points(WALL_1_START, WALL_1_END, WALL_STEP)
const WALL_POINTS_2 = create_wall_points(WALL_2_START, WALL_2_END, WALL_STEP)
const WALL_POINTS_4 = create_wall_points(WALL_4_START, WALL_4_END, WALL_STEP)
const WALL_POINTS_6 = create_wall_points(WALL_6_START, WALL_6_END, WALL_STEP)
const WALL_POINTS_8 = create_wall_points(WALL_8_START, WALL_8_END, WALL_STEP)
const WALL_POINTS_10 = create_wall_points(WALL_10_START, WALL_10_END, WALL_STEP)
const WALL_POINTS_12 = create_wall_points(WALL_12_START, WALL_12_END, WALL_STEP)
const WALL_POINTS_14 = create_wall_points(WALL_14_START, WALL_14_END, WALL_STEP)
const WALL_POINTS_16 = create_wall_points(WALL_16_START, WALL_16_END, WALL_STEP)
const WALL_POINTS_18 = create_wall_points(WALL_18_START, WALL_18_END, WALL_STEP)
const WALL_POINTS_20 = create_wall_points(WALL_20_START, WALL_20_END, WALL_STEP)
const WALL_POINTS_22 = create_wall_points(WALL_22_START, WALL_22_END, WALL_STEP)

# Concateniamo tutto in un'unica lista che il VMS userà
const OBSTACLES_LIST = vcat( WALL_POINTS_1, WALL_POINTS_2, WALL_POINTS_4, WALL_POINTS_6, WALL_POINTS_8, WALL_POINTS_10, WALL_POINTS_12, WALL_POINTS_14, WALL_POINTS_16, WALL_POINTS_18, WALL_POINTS_20, WALL_POINTS_22)

println("Generati ostacoli a muro. Totale punti repulsivi: $(length(OBSTACLES_LIST))")

# --- PARAMETRI SNAKE DINAMICI ---
const MAX_SEGMENTS = 15        # Limite di sicurezza per evitare loop infiniti
const L_SEGMENT_DEFAULT = 0.15  # Lunghezza nominale segmento
...
while !reached_target && num_generated_segments < MAX_SEGMENTS
    ...
    add_component!(vm_gen, LinearDamper(0.1 * I(3), "Gen_Vel$(i)"); id="Gen_RotDamp$(i)") ################################################à
...
# Definisci un coefficiente di smorzamento Cartesiano (da tarare, es. 5.0 o 10.0)
CONST_D_CARTESIAN_3D = 1.0 

for i in 1:num_generated_segments
    seg_pos_id = ".virtual_mechanism.Seg$(i)_Pos"
    
    # CORREZIONE: Creiamo esplicitamente una SMatrix 3x3 diagonale
    # Questo soddisfa i requisiti di performance di VMRobotControl
    D_matrix = SMatrix{3, 3, Float64}(
        CONST_D_CARTESIAN_3D, 0.0, 0.0,
        0.0, CONST_D_CARTESIAN_3D, 0.0,
        0.0, 0.0, CONST_D_CARTESIAN_3D
    )
    
    add_component!(vms, LinearDamper(D_matrix, seg_pos_id); id="Seg$(i)_Cartesian_Damper")
end
...
#add_component!(vms, TanhSpring("Snake_Tip_Ref_Err"; stiffness=K_ATTR_TARGET, max_force=10.0); id="Snake_Tip_Anchor_Spring")
...
links_robot = ["fr3_link2", "fr3_link3", "fr3_link4", "fr3_link5", "fr3_link6", "fr3_link7", "fr3_link8", "fr3_hand"]
...
# Visualizzazione Statica (MODIFICATA)
mesh!(ls, Sphere(Point3f(PIVOT_POINT), 0.03), color=:green, label="Pivot")

# Disegna i "mattoni" del muro più piccoli
for (k, obs) in enumerate(OBSTACLES_LIST)
    # Riduco il raggio da 0.04 a 0.02 per pulizia visiva
    mesh!(ls, Sphere(Point3f(obs), 0.025), color=:orange) 
end
# Aggiungo una entry fittizia per la legenda (altrimenti avresti 50 voci "Ostacolo k")
poly!(ls, Point3f[]; color=:orange, label="Muri (Repulsori)")

# Visualizzazione Piano con Buco (Wall Visualization)
color_wall = (:gray, 0.3)
# Parte Sinistra (Y <= 0)
mesh!(ls, Rect3f(Vec3f(0.395, -0.7, 0.0), Vec3f(0.01, 0.7, 0.9)), color=color_wall, transparency=true)
# Parte Destra (Y >= 0.4)
mesh!(ls, Rect3f(Vec3f(0.395, 0.4, 0.0), Vec3f(0.01, 0.3, 0.9)), color=color_wall, transparency=true)
# Parte Sotto (0 < Y < 0.4, Z <= 0.3)
mesh!(ls, Rect3f(Vec3f(0.395, 0.0, 0.0), Vec3f(0.01, 0.4, 0.3)), color=color_wall, transparency=true)
# Parte Sopra (0 < Y < 0.4, Z >= 0.7)
mesh!(ls, Rect3f(Vec3f(0.395, 0.0, 0.7), Vec3f(0.01, 0.4, 0.2)), color=color_wall, transparency=true)
