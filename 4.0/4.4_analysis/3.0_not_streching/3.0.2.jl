...
const TARGET_TOLERANCE = SEGMENT_LENGTH    # La soglia di arrivo è uguale alla lunghezza del segmento
...
function vector_to_yaw_pitch(v)
    ...
    # MODIFICA: Segmenti lungo +Z
    # Yaw (Z) -> Pitch (X) -> Segment (Z)
    
    yaw = atan(v_norm[1], -v_norm[2]) 
    
    horiz = sqrt(v_norm[1]^2 + v_norm[2]^2)
    
    # Pitch per allineare Z al vettore
    pitch = atan(horiz, v_norm[3])
...
const K_ATTR_GEN = 80.0   
const K_REPEL_GEN = -70.0
const W_REPEL_GEN = 0.1  
const DAMPING_MOVING = 0.1
...
for i in 1:MAX_SEGMENTS
    ...
    add_joint!(vm_gen, Rigid(Transform(SVector(0.0, 0.0, SEGMENT_LENGTH))); parent=j_frame, child=s_frame, id="Gen_Seg_Rigid$(i)")
    ...
    for (k, obs) in enumerate(OBSTACLES_LIST)
        ...
        add_component!(vm_gen, GaussianSpring("Gen_Err_Obs$(i)_$(k)"; stiffness=K_REPEL_GEN, width=W_REPEL_GEN); id="Gen_Repel$(i)_$(k)")
    end
...
segment_inertia_val = (1 / 12) * 0.5 * SEGMENT_LENGTH^2 # Massa unitaria approx
...
for i in 1:NUM_ACTUAL_SEGMENTS
    ...
    if i == 1
        ...
        # Link rigido
        add_joint!(vm, Rigid(Transform(SVector(0.0, 0.0, SEGMENT_LENGTH))); parent=joint_frame, child=seg_frame, id="Link$(i)_Rigid")
    else
...
for (k, obs) in enumerate(OBSTACLES_LIST)
    ...
    for i in 1:NUM_ACTUAL_SEGMENTS
        ...
        add_component!(vms, GaussianSpring(err_id; stiffness=K_REPEL_GEN, width=W_REPEL_GEN); id="Seg$(i)_Spr$(k)") 
    end
end

# Repulsione Robot (Invariata)
links_robot = ["fr3_link4", "fr3_link5", "fr3_link6", "fr3_link7", "fr3_link8", "fr3_hand"]
ostacoli_robot_cfg = [(id=k, c=".virtual_mechanism.ObstaclePosition$k") for k in 1:length(OBSTACLES_LIST)]
for lnk in links_robot
    ...
    for obs in ostacoli_robot_cfg
        ...
        add_component!(vms, GaussianSpring("$(lnk)_ObsErr$(obs.id)"; stiffness=-350.0, width=0.1); id="$(lnk)_ObsSpr$(obs.id)")
    end
end
...
add_component!(vms, LinearSpring(K_ATTR_GEN, "Snake_Tip_Anchor_Error"); id="Snake_Tip_Constraint_Spring")
add_component!(vms, LinearDamper(10.0, "Snake_Tip_Anchor_Error"); id="Snake_Tip_Constraint_Damper")
=#
...
function f_control(cache, t, args, extra)
    ...
    # Ostacolo 3: Movimento asse Y a mezz'aria
    if length(obs_refs) >= 3
        obs_refs[3][] = OBSTACLE_POSITION_3 + SVector(0.0, delta_val, 0.0)
    end
...
# Visualizzazione Ostacolo 3 (Mobile)
obs3_id = get_compiled_coordID(compile(vms), ".virtual_mechanism.ObstaclePosition3")
obs3_pos = map(c -> Point3f(configuration(c, obs3_id)), pkcache)
mesh!(ls, map(p -> Sphere(p, 0.04), obs3_pos), color=:cyan, label="Ostacolo 3")
